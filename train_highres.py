"""
High-resolution multitask training.
  - hop_length=256  → TARGET_FRAMES=126 (2x time resolution vs baseline)
  - Modest real augmentation (2-5x minority classes from real audio)
  - Original proven focal_loss gamma=4, no asymmetric loss
  - Val/test: real data only

Run after reextract_features.py and augment_modest.py:
...
LR            = 1e-3
LR_MIN        = 1e-6
TARGET_FRAMES = 126   # hop_length=256: 1+floor(32000/256)=126 frames for 2s
N_MELS        = 128
DIAG_WEIGHT   = 0.1
...
class ICBHICallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if score > self.best:
            self.best = score
            self.model.save(f'{CKPT_DIR}/highres_best.keras')
            print(f'  val_icbhi: {score:.4f} *** NEW BEST ***')
        else:
            print(f'  val_icbhi: {score:.4f}')


# ── Load manifest ──────────────────────────────────────────────
aug_csv = 'data/processed/manifest_aug_modest.csv'
if not os.path.exists(aug_csv):
    print("ERROR: manifest_aug_modest.csv not found.")
    print("Run:  python augment_modest.py  first.")
    raise SystemExit(1)

manifest = pd.read_csv(aug_csv)
train_df = manifest[manifest['split'] == 'train']
val_df   = manifest[(manifest['split'] == 'val') & (manifest['source'].fillna('real') == 'real')]

print(f"Train: {len(train_df)}  (incl. augmented)")
...
model_best = tf.keras.models.load_model(f'{CKPT_DIR}/highres_best.keras', compile=False)
...
axes[0].set_title('Sound Accuracy (highres)')
axes[0].set_xlabel('Epoch'); axes[0].legend()

axes[1].plot(history.history['loss'],     label='Train Loss')
axes[1].plot(history.history['val_loss'], label='Val Loss')
axes[1].set_title('Total Loss'); axes[1].set_xlabel('Epoch'); axes[1].legend()
...
axes[2].set_title('ICBHI Score (highres)'); axes[2].set_xlabel('Epoch'); axes[2].legend()

plt.tight_layout()
plt.savefig(f'{RESULTS_DIR}/highres_training_curves.png', dpi=150)
...
axes[0].set_title('Sound (highres)'); axes[0].set_ylabel('True'); axes[0].set_xlabel('Predicted')
...
axes[1].set_title('Diagnosis (highres)'); axes[1].set_ylabel('True'); axes[1].set_xlabel('Predicted')

plt.xticks(rotation=45, ha='right'); plt.tight_layout()
plt.savefig(f'{RESULTS_DIR}/highres_confusion_matrices.png', dpi=150)
