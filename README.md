...
- **`scripts/train_multitask.py` updated for the merged dataset**: diagnosis-loss masking for HF_Lung rows (no diagnosis labels), batched val/test inference (avoids GPU OOM from a single unbatched forward pass), full/icbhi-only/hf_lung-only test-set evaluation. An unweighted merge regressed ICBHI-only test performance to 51.96% (below the 62.26% baseline) despite a strong combined score, since HF_Lung is ~90% of training rows.
- **Fixed a Keras 3 crash** (`KeyError: 0` in `compile_utils.py`'s `resolve_path`, reproduced locally and confirmed 100% deterministic): passing per-output `sample_weight` as a **dict** to `model.fit()` against this project's list-output multitask model (`[sound_out, diag_out]`) crashes on this Keras version regardless of architecture, batch size, or `validation_data`. Fix: pass both `y` and `sample_weight` as **ordered lists** matching `model.output_names` instead of dicts.
- **Source-aware sample weighting tried and abandoned**: multiplying `class_weight × ICBHI_SOURCE_WEIGHT(9.0)` compounded in the extreme cell (ICBHI's "Both" class, weight ≈16.55 × 9.0 ≈149x), teaching the model to over-predict "Both" on ICBHI-like inputs instead of learning ICBHI's general acoustics better — ICBHI-only barely moved (+0.09pt) while hf_lung-only and full both got worse (-9.38pt, -6.27pt). Replaced with **stratified batch sampling** (`ICBHI_BATCH_FRACTION`): every training batch draws a fixed, verified-deterministic count from separate ICBHI/HF_Lung pools (not sample-weight multiplication), so ICBHI gets guaranteed per-batch representation without distorting any individual sample's loss. Existing per-class weighting is unchanged. Checkpointed as `multitask_batchstratified_*`, alongside `multitask_combined_*` and `multitask_sourceweighted_*`, so all runs stay on disk for comparison.
